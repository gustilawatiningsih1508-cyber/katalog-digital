    ...
    <div class="flex overflow-hidden bg-white pt-16">
          ...
          <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
            <div class="flex-1 px-3 bg-white divide-y space-y-1">
              <ul class="space-y-2 pb-2">
                ...
                <li>
                  <a href="/users" class="text-base text-gray-900 font-normal rounded-lg hover:bg-gray-100 flex items-center p-2 group bg-gray-100">
                    <svg class="w-6 h-6 text-gray-900 flex-shrink-0 group-hover:text-gray-900 transition duration-75" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"></path></svg>
              ...
              <div class="p-4 border-t border-gray-700 mt-auto">
                <a href="/" class="block py-2 px-3 rounded bg-red-600 text-center hover:bg-red-700 text-white">
                  Logout
                </a>
              </div>
            </div>
      ...
      <div id="main-content" class="h-full w-full bg-gray-50 relative overflow-y-auto lg:ml-64">
        <main>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                      ...
                      <tr>
                        ...
                        <td class="p-4 text-sm font-medium">
                          <button type="button" data-modal-toggle="edit-user-modal-{{ $user->id }}" class="text-cyan-600 hover:text-cyan-900 mr-3">Edit</button>
                          <button type="button" data-modal-toggle="delete-user-modal-{{ $user->id }}" class="text-red-600 hover:text-red-900">Hapus</button>
                        </td>
                      </tr>
                      @empty
                      <tr>
                  ...
                  <div class="p-6 space-y-6">
                    <div class="grid grid-cols-6 gap-6">
                      <div class="col-span-6 sm:col-span-3">
                        <label for="edit-username-{{ $user->id }}" class="text-sm font-medium text-gray-900 block mb-2">Username</label>
                        <input type="text" name="username" id="edit-username-{{ $user->id }}" value="{{ $user->username }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required>
                      </div>
                      <div class="col-span-6 sm:col-span-3">
                        <label for="edit-email-{{ $user->id }}" class="text-sm font-medium text-gray-900 block mb-2">Email</label>
                        <input type="email" name="email" id="edit-email-{{ $user->id }}" value="{{ $user->email }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required>
                      </div>
                      <div class="col-span-6 sm:col-span-3">
                        <label for="edit-password-{{ $user->id }}" class="text-sm font-medium text-gray-900 block mb-2">Password (Kosongkan jika tidak diubah)</label>
                        <input type="password" name="password" id="edit-password-{{ $user->id }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" placeholder="••••••••">
                      </div>
                      <div class="col-span-6 sm:col-span-3">
                        <label for="edit-hak-akses-{{ $user->id }}" class="text-sm font-medium text-gray-900 block mb-2">Hak Akses</label>
                        <select name="hak_akses" id="edit-hak-akses-{{ $user->id }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required>
                          <option value="1" {{ $user->hak_akses == 1 ? 'selected' : '' }}>Super Admin</option>
                          <option value="2" {{ $user->hak_akses == 2 ? 'selected' : '' }}>Penjual</option>
                        </select>
                      </div>
                      <div class="col-span-6">
                        <label for="edit-laporan-aktifitas-{{ $user->id }}" class="text-sm font-medium text-gray-900 block mb-2">Laporan Aktifitas</label>
                        <textarea name="laporan_aktifitas" id="edit-laporan-aktifitas-{{ $user->id }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5">{{ $user->laporan_aktifitas }}</textarea>
                      </div>
                    </div>
                  </div>
          ...
          @foreach($users as $user)
          <div class="hidden overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full" id="delete-user-modal-{{ $user->id }}">
              ...
              <div class="bg-white rounded-lg shadow relative">
                ...
                <form action="{{ route('users.destroy', $user->id) }}" method="POST">
                  ...
                  <div class="p-6 pt-0 text-center">
                    ...
                    <h3 class="text-xl font-normal text-gray-500 mt-5 mb-6">Apakah Anda yakin ingin menghapus user ini?</h3>
                    ...
                    <button type="button" class="text-gray-900 bg-white hover:bg-gray-100 focus:ring-4 focus:ring-cyan-200 border border-gray-200 font-medium inline-flex items-center rounded-lg text-base px-3 py-2.5 text-center" data-modal-toggle="delete-user-modal-{{ $user->id }}">
                      Tidak, Batal
                    </button>
                  </div>
                </form>
                ...
                <form action="{{ route('users.store') }}" method="POST">
                  ...
                  <div class="p-6 space-y-6">
                    <div class="grid grid-cols-6 gap-6">
                      ...
                      <div class="col-span-6 sm:col-span-3">
                        <label for="password_confirmation" class="text-sm font-medium text-gray-900 block mb-2">Konfirmasi Password</label>
                  ...
                  <div class="p-6 border-t border-gray-200 rounded-b">
                    <button type="submit" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Add User</button>
                  </div>
                </form>
              </div>
            </div>
