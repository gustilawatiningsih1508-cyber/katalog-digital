        ...
        <main>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                      ...
                      <tr>
                        ...
                        <td class="p-5 text-sm font-medium">
                          <button type="button" data-modal-toggle="edit-product-modal-{{ $product->id_produk }}" class="text-cyan-600 hover:text-cyan-900 mr-3">Edit</button>
                          <button type="button" data-modal-toggle="delete-product-modal-{{ $product->id_produk }}" class="text-red-600 hover:text-red-900">Hapus</button>
                        </td>
                      </tr>
                      @endforeach
                    </tbody>
          ...
          <div class="bg-white sticky sm:flex items-center w-full sm:justify-between bottom-0 right-0 border-t border-gray-200 p-4">
            <div class="flex items-center mb-4 sm:mb-0">
              ...
              <span class="text-sm font-normal text-gray-500">Showing <span class="text-gray-900 font-semibold">1-{{ $products->count() }}</span> of <span class="text-gray-900 font-semibold">{{ $products->count() }}</span></span>
            </div>
          ...
          @foreach($products as $product)
          <div class="hidden overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full" id="edit-product-modal-{{ $product->id_produk }}">
            <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
              <div class="bg-white rounded-lg shadow relative">
                <div class="flex items-start justify-between p-5 border-b rounded-t">
                  <h3 class="text-xl font-semibold">Edit product</h3>
                  <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="edit-product-modal-{{ $product->id_produk }}">
                  ...
                  <div class="p-6 space-y-6">
                    <div class="grid grid-cols-6 gap-6">
                      <div class="col-span-6 sm:col-span-3">
                        <label for="edit-id-pelaku-usaha-{{ $product->id_produk }}" class="text-sm font-medium text-gray-900 block mb-2">Id Pelaku Usaha</label>
                        <input type="text" name="id_pelaku_usaha" id="edit-id-pelaku-usaha-{{ $product->id_produk }}" value="{{ $product->id_pelaku_usaha }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required>
                      </div>
                      <div class="col-span-6 sm:col-span-3">
                        <label for="edit-nama-produk-{{ $product->id_produk }}" class="text-sm font-medium text-gray-900 block mb-2">Nama Produk</label>
                        <input type="text" name="nama_produk" id="edit-nama-produk-{{ $product->id_produk }}" value="{{ $product->nama_produk }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required>
                      </div>
                      <div class="col-span-6 sm:col-span-3">
                        <label for="edit-harga-{{ $product->id_produk }}" class="text-sm font-medium text-gray-900 block mb-2">Harga</label>
                        <input type="number" name="harga" id="edit-harga-{{ $product->id_produk }}" value="{{ $product->harga }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required>
                      </div>
                      <div class="col-span-6 sm:col-span-3">
                        <label for="edit-kategori-{{ $product->id_produk }}" class="text-sm font-medium text-gray-900 block mb-2">Kategori</label>
                        <input type="text" name="kategori" id="edit-kategori-{{ $product->id_produk }}" value="{{ $product->kategori }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required>
                      </div>
                      <div class="col-span-6">
                        <label for="edit-deskripsi-{{ $product->id_produk }}" class="text-sm font-medium text-gray-900 block mb-2">Deskripsi</label>
                        <textarea name="deskripsi" id="edit-deskripsi-{{ $product->id_produk }}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5">{{ $product->deskripsi }}</textarea>
                      </div>
                    </div>
                  </div>
          ...
          <!-- MODAL HAPUS UNTUK SETIAP PRODUK -->
          @foreach($products as $product)
          <div class="hidden overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full" id="delete-product-modal-{{ $product->id_produk }}">
            <div class="relative w-full max-w-md px-4 h-full md:h-auto">
              <div class="bg-white rounded-lg shadow relative">
                <div class="flex justify-end p-2">
                  <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="delete-product-modal-{{ $product->id_produk }}">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>  
                  </button>
                </div>
                <form action="{{ route('products.destroy', $product->id_produk) }}" method="POST">
                  ...
                  <div class="p-6 pt-0 text-center">
                    ...
                    <button type="button" class="text-gray-900 bg-white hover:bg-gray-100 focus:ring-4 focus:ring-cyan-200 border border-gray-200 font-medium inline-flex items-center rounded-lg text-base px-3 py-2.5 text-center" data-modal-toggle="delete-product-modal-{{ $product->id_produk }}">
                      No, cancel
                    </button>
                  </div>
                </form>
              ...
              <div class="bg-white rounded-lg shadow relative">
                ...
                <form action="{{ route('products.store') }}" method="POST">
                  ...
                  <div class="p-6 space-y-6">
                    <div class="grid grid-cols-6 gap-6">
                      <div class="col-span-6 sm:col-span-3">
                        <label for="id_pelaku_usaha" class="text-sm font-medium text-gray-900 block mb-2">Id Pelaku Usaha</label>
                        <input type="text" name="id_pelaku_usaha" id="id_pelaku_usaha" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required>
                      </div>
                      <div class="col-span-6 sm:col-span-3">
                        <label for="nama_produk" class="text-sm font-medium text-gray-900 block mb-2">Nama Produk</label>
                        <input type="text" name="nama_produk" id="nama_produk" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required>
                      </div>
                      <div class="col-span-6 sm:col-span-3">
                        <label for="harga" class="text-sm font-medium text-gray-900 block mb-2">Harga</label>
                        <input type="number" name="harga" id="harga" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required>
                      </div>
                      <div class="col-span-6 sm:col-span-3">
                        <label for="kategori" class="text-sm font-medium text-gray-900 block mb-2">Kategori</label>
                        <input type="text" name="kategori" id="kategori" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" required>
                      </div>
                      <div class="col-span-6">
                        <label for="deskripsi" class="text-sm font-medium text-gray-900 block mb-2">Deskripsi</label>
                        <textarea name="deskripsi" id="deskripsi" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"></textarea>
                      </div>
                    </div>
                  </div>
                  <div class="p-6 border-t border-gray-200 rounded-b">
                    <button type="submit" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Tambah Produk</button>
                  </div>
                </form>
              </div>
            </div>
